...
* Covered Tg API [`10.2`](https://core.telegram.org/bots/api-changelog#july-14-2026): ephemeral messages (`receiverUserId`/`callbackQueryId` options on all send actions, `editEphemeralMessageText`/`Media`/`Caption`/`ReplyMarkup`, `deleteEphemeralMessage`, `Message.receiverUser`/`ephemeralMessageId`, `ReplyParameters.ephemeralMessageId`, `BotCommand.isEphemeral`, `Update.subscription` with `BotSubscriptionUpdated`); block-based rich message input (`InputRichMessage.blocks`/`media`, `InputRichMessageMedia`, `InputMediaVoiceNote`); communities (`Community`, `CommunityChatAdded`, `CommunityChatRemoved`, `ChatFullInfo.community`, `Message.communityChatAdded`/`communityChatRemoved`).
* `ReplyParameters.messageId` is now optional (nullable) since a reply target can be specified via `ephemeralMessageId` instead.
