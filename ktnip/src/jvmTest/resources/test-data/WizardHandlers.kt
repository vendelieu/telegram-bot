...
object TestWizard {
    ...
    object NameStep : WizardStep(isInitial = true) {
        ...
        override suspend fun validate(ctx: WizardContext): Transition {
            val text = ctx.update.safeCast<MessageUpdate>()?.message?.text ?: return Transition.Retry()
        ...
        override suspend fun store(ctx: WizardContext): String? =
            ctx.update.safeCast<MessageUpdate>()?.message?.text
    }
    ...
    object AgeStep : WizardStep() {
        ...
        override suspend fun store(ctx: WizardContext): Int? =
            ctx.update.safeCast<MessageUpdate>()?.message?.text?.toIntOrNull()
    }
    ...
    object ConfirmStep : WizardStep() {
        ...
        override suspend fun validate(ctx: WizardContext): Transition {
            val text = ctx.update.safeCast<MessageUpdate>()?.message?.text?.lowercase() ?: return Transition.Retry()
            return when (text) {
